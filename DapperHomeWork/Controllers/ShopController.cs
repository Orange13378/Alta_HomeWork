...
public class ShopController : ControllerBase
{
    ...
    {
        if (shop == null)
            return BadRequest("ƒанные не могут быть пустыми");

        var shops = _shopRepository.GetAll();

        if (shops.Any(login => shop.Login == login.Login))
            return BadRequest("ћагазин с таким логином уже существует");

        var rows = _shopRepository.Insert(shop);

        return rows switch
        {
            null => BadRequest("“ип должен быть либо 1 либо 2"),
            false => BadRequest("Ќе удалось создать магазин"),
            _ => Ok() 
        };
    }
    ...
    public IActionResult Update(int id, [FromBody] UpdateShop? shop)
    {
        ...
        if (shop == null)
            return BadRequest("ѕустой запрос на обновление");
