...
namespace xUnitTest
{
    public class UserControllerUnitTest
    {
        ...
        public void Authorize_ValidCredentials_ReturnsOkObjectResultWithTokenTest()
        {
            ...
            var existingUser = new User { UserName = "randomUsername", Password = "$2a$11$sFdNOdG4WJcz2pnyTK1IZOMKbNGarCaKLZYEydjAusVhmVL21w8Xe", Role = "randomRole" };

            _mockUserRepository.Setup(repo => repo.GetUserByLogin(auth.Login)).Returns(existingUser);
            ...
            Assert.Contains(jwt.Claims, claim => claim.Type == ClaimsIdentity.DefaultNameClaimType && claim.Value == existingUser.UserName);
            Assert.Contains(jwt.Claims, claim => claim.Type == ClaimsIdentity.DefaultRoleClaimType && claim.Value == existingUser.Role);
        }
        ...
        public void Register_ValidUser_ReturnsCreatedResultTest()
        {
            ...
            _mockUserRepository.Setup(repo => repo.GetUserByLogin(user.UserName)).Returns((User)null);
            _mockUserRepository.Setup(repo => repo.GetShopById(user.ShopId)).Returns((Shop)null);
        ...
        public void Register_UserWithExistingLogin_ReturnsBadRequestTest()
        {
            ...
            _mockUserRepository.Setup(repo => repo.GetUserByLogin(user.UserName)).Returns(existingUser);
        ...
        public void Register_UserWithInvalidShopId_ReturnsBadRequestTest()
        {
            ...
            _mockUserRepository.Setup(repo => repo.GetUserByLogin(user.UserName)).Returns((User)null);
            _mockUserRepository.Setup(repo => repo.GetShopById(user.ShopId)).Returns(existingShop);
